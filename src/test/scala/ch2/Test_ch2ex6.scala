...
class Test_ch2ex6 extends FlatSpec
  with MustMatchers
  with Timeouts
  ...
  def producerBody(numbIteration: Int, sleepBaseTime: Long, sync: SyncQueue[Int], producedElements: ArrayBuffer[Int]): Unit = {
  ...
  def consumerBody(numbIteration: Int, sleepBaseTime: Long, sync: SyncQueue[Int], consumedElements: ArrayBuffer[Int]): Unit = {
  ...
  it should "works using Scala Check properties" in {
    ...
    case class Scenario( queueSize: Int, consumerBefore: Boolean, numberExchanged: Int, producerBaseSleep: Int, consumerBaseSleep: Int)
    
    lazy val genScenario: Gen[Scenario] = for {
      queueSize               <- Gen.choose[Int](1, 10)
    ...
    } yield Scenario( queueSize, consumerStartsBefore, numberExchanged, producerBaseSleep, consumerBaseSleep)
    
    forAll(genScenario) { (scenario) => {
      
        log(scenario.toString)
      
        val sync: SyncQueue[Int] = new SyncQueue(scenario.queueSize)
